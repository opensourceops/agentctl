...
Combine deterministic gate results and package digest evidence into an explicit go/no-go decision.
...
python3 examples/devops/run.py --agentctl target/debug/agentctl --only 10 --keep --report /tmp/agentctl-devops-10.json
```
...
The runner validates the case-specific structured report and its source-derived fields. Expected no-go is a successful analysis result, not launch approval: security-review is deliberately unresolved.
