# 14. Approval-gated local deployment

Pause before changing a disposable local HTTP service document, approve the exact effect, resume, and probe the service.
...
python3 examples/devops/run.py --agentctl target/debug/agentctl --only 14 --keep --report /tmp/agentctl-devops-14.json
```
...
The runner starts an ephemeral-port loopback HTTP server, proves the old version while approval is pending, approves with devops-fixture-reviewer, resumes, verifies the new version, and always shuts the server down.
