# Remediate a scanned container dependency
...
The preflight and implementer explicitly set `providerOptions.toolStrict: false`. This disables OpenAI’s constrained tool-argument generation for these exact multiline schemas; agentctl still validates every returned argument against the complete path, content pattern and length schema before dispatch, and still enforces each agent’s tool scope and workflow policy. Final structured outputs remain strict. Other workflows keep strict tool generation by default. The live comparison succeeded with this one provider option changed after constrained generation repeatedly returned an incomplete response; this is compatibility evidence, not a guarantee that fresh model responses are deterministic.

The default operation is `remediate`. The authored runtime ceilings are four provider requests, 20,000 total input-plus-output tokens, 600 seconds and US$1 estimated cost. The analyzer has one turn; the implementer has at most three turns and two tool calls. Their per-response output caps are 2,048 tokens for the analyzer and 4,096 for the implementer. The coordinator also reserves aggregate allowance before dispatch, reconciles actual durable usage and retains uncertain reservations. Runtime ceilings must fit the assigned lease. The dated pricing estimate includes uncached input, cached input, cache writes and output. It is recorded in the workflow, not presented as a guaranteed invoice.
