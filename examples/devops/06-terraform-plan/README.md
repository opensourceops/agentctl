# 6. Governed Terraform plan analysis

Reject destructive or nonlocal plan changes and require recorded approval before writing isolated local state.
...
python3 examples/devops/run.py --agentctl target/debug/agentctl --only 06 --keep --report /tmp/agentctl-devops-06.json
```
...
The runner inspects each pending effect, approves with actor devops-fixture-reviewer and an explicit reason, and resumes. It verifies local-state.json is absent before the apply-local approval. Changing policy to never would invalidate this example.
