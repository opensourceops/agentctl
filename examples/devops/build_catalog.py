...
for number,(slug,title,description) in enumerate(TITLES,1):
    ...
    if number==1:
        ...
        spec['actions']['read']={'kind':'builtin.read'}
        spec['providers']={'fake':{'kind':'fake'}}
        spec['tools']={'repair_config':{'kind':'builtin.workspace.write','description':'Stage the exact approved decimal timeout value','inputSchema':obj({'path':{'type':'string','enum':['artifacts/timeout-seconds.txt']},'content':{'type':'string','enum':['30']}}),'outputSchema':{'type':'object'},'capability':'filesystem.write','effectClass':'workspace_mutate','risk':'medium','idempotency':'idempotent','retrySafe':True,'timeoutSeconds':5,'approval':'policy'}}
        spec['agents']={'remediator':agent(folder,'remediator','Inspect the provided configuration; the allowed timeout is 30. Correct the oversized timeout by calling repair_config exactly once with path artifacts/timeout-seconds.txt and content exactly the two ASCII characters 30, with no whitespace or newline. Then return done=true and timeoutSeconds=30. The completion flag stops the bounded loop. After successful completion, a deterministic helper validates the exact staged bytes and serializes the approved value into artifacts/remediation.json.',{'done':True,'timeoutSeconds':30},['repair_config'],{'path':'artifacts/timeout-seconds.txt','content':'30'})}
