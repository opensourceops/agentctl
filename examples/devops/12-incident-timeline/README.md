...
Sort timestamped incident logs, calculate incident duration, and validate a model recommendation against source references.
...
python3 examples/devops/run.py --agentctl target/debug/agentctl --only 12 --keep --report /tmp/agentctl-devops-12.json
```
...
```sh
python3 examples/devops/run.py --agentctl target/debug/agentctl --only 12 --mode openai --model gpt-5-mini --live-budget /tmp/agentctl-launch-live-budget.sqlite3 --report /tmp/agentctl-devops-12-live.json
```
