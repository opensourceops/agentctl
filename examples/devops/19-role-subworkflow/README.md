# 19. Typed role sub-workflow

Connect planner, reviewer, and executor through typed handoffs in a reusable sub-workflow with distinct tool visibility.
...
python3 examples/devops/run.py --agentctl target/debug/agentctl --only 19 --keep --report /tmp/agentctl-devops-19.json
```
...
```sh
python3 examples/devops/run.py --agentctl target/debug/agentctl --only 19 --mode openai --model gpt-5-mini --live-budget /tmp/agentctl-launch-live-budget.sqlite3 --report /tmp/agentctl-devops-19-live.json
```
