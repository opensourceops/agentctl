...
### Windows execution and completed live inventory checkpoint

Hosted Windows CI33998511938 actually passed all four source-confinement tests, including mutation of the already opened directory, and all twenty deterministic examples on db7b59b. Linux/macOS full CI and RC checks passed. Windows then failed at temporary-directory cleanup in all six Python shared-budget tests: their SQLite handles remained open (WinError32). This is a test/harness resource-lifecycle defect, not a failed source-confinement assertion or an environmental exemption. The next increment closes owned ledgers on normal/error exits and constructor failure, preserves every accounting assertion, and tests closed handles plus immediate deletion and retained reservations.

Clean source `9c4f6e0c84c8811e6fb015a7b43c311b7de67d73` passed all twenty deterministic cases. Its focused live20 run passed two requests, 772 input+output tokens, estimated423 microUSD and four seconds, with exact staging/final JSON bytes, one iteration, validated completion, and keyless zero-effect replay. Together with unchanged01/12/19 successes at db7b59b, all four distinct new OpenAI variants now have actual passing evidence. No whole previously failed invocation is relabeled as passing. The completed Astra and Sol provider/tool/composite/recovery coverage remains source-labeled and is not repeated.

Final paid accounting before any new authorized paid change: known reconciled45 requests/10,367 tokens/113 seconds/44,363 microUSD estimate; unresolved prior reservation12 requests/25,536 tokens/240 seconds/2,000,000 microUSD; allowance charged57 requests/35,903 tokens/353 seconds/2,044,363 microUSD. By model, known mini14 requests/4,822 tokens/4,377 microUSD; Sol27 requests/4,492 tokens/26,816 microUSD; Astra4 requests/1,053 tokens/13,170 microUSD. The extra reserved Sol usage remains unknown, not zero or actual. No further paid rerun is required solely for connection cleanup or evidence recording.

Next: finish owned-connection cleanup and deterministic wrapper regression, commit/push final source, complete final hosted Linux/macOS/Windows/container/security/package/SBOM/RC gates, pin/build/verify docs to that exact source, then commit evidence-only files separately and update linked draft PRs. No merge, tag, package publication, public-site deployment or production mutation is authorized.

Connection-lifecycle verification passed before final source push: seven shared-budget tests, ten fixture tests, nineteen wrapper tests and the OCI adapter test. The new wrapper failure case raises after dispatch/output handling, then proves the connection is closed, the full committed reservation remains charged, and the temporary database can be deleted immediately. Existing request/usage/uncertainty assertions remain unchanged. The real shared paid ledger was not modified by these credential-free fixtures. Documentation freshness/link checks and secret scan pass.


### Final source, verified artifacts and release verdict — 6 September 2026

**Launch-ready pre-1.0 candidate for review**, with no unresolved required release gate. This is not a stable 1.0 release or authorization to merge, publish or deploy. Final executable source is `ce94443a6b0a840eb9cd0e5f5b2e128dfe8f15c8`. The later evidence-only commit changes this ledger, evidence matrices and JSON reports; it does not change Rust, Python, workflows, fixtures, schema, dependencies or imported site content. Its SHA is recorded in the draft PR and final task report rather than self-embedded here.

The owned SQLite connection correction is now fully verified on actual Windows. All required final-source hosted workflows passed:

| Gate | Run | Result |
| --- | --- | --- |
| Credential-free CI, Linux x86_64 / macOS arm64 / Windows x86_64, and production SBOM | [33999337160](https://github.com/opensourceops/agentctl/actions/runs/33999337160) | Passed all jobs |
| RC preparation and packages on all three targets | [33999340847](https://github.com/opensourceops/agentctl/actions/runs/33999340847) | Passed all jobs |
| Container acceptance, image security and image SBOM | [33999337167](https://github.com/opensourceops/agentctl/actions/runs/33999337167) | Passed |
| Supply-chain security | [33999337162](https://github.com/opensourceops/agentctl/actions/runs/33999337162) | Passed |
| Paired documentation, pinned to the exact framework source | [33999609203](https://github.com/opensourceops/opensourceops.github.io/actions/runs/33999609203) | Passed; deployment skipped |

CI and RC executed `cargo xtask verify`, `cargo xtask acceptance`, `cargo xtask completeness` and `cargo xtask package`; CI also explicitly ran `cargo xtask docs-verify`. The verify gate includes full workspace tests, formatting/Clippy, generated freshness, exact example inventory, all twenty isolated DevOps examples and credential-free budget/fixture tests, security and production-boundary checks. Artifact-store, migration and protocol-resilience aliases select tests already executed by that full suite. Container acceptance and secret/image/dependency security have their separate applicable hosted evidence. All four adversarial Windows source-confinement tests passed in the complete final gate.

`hosted-final-ce94443.json` independently verifies eight downloaded GitHub artifact ZIP digests against the API, all inner package `SHA256SUMS` entries, and both CycloneDX documents. The production SBOM contains 279 components (file SHA256 `f4b9d247e488828796b5a189e829d4193275e07ec85d8eb63a2bc5af0178fed8`). The image SBOM contains 12 components and records imageID `sha256:a757f1ed75b30cc4cf243da4086a27a513531d637be1be47e3cab4cd50aa4b09`. Full per-platform ZIP/file hashes, artifact IDs and run-source bindings are checked in as [LAUNCH_FINAL_EVIDENCE.json](LAUNCH_FINAL_EVIDENCE.json). These are separately built CI/RC packages, not a claim of byte-identical reproducible builds.

All twenty cases passed locally from isolated clean directories at `ce94443a6b0a840eb9cd0e5f5b2e128dfe8f15c8` in `devops-final-ce94443.json`. Case 04's actual disposable OCI build also passed at that source in `devops-container-final-ce94443.json`, using pinned base `docker.io/library/python@sha256:3949e4271b0a3ff82afac7306764c313dcc8edeeb89c0376a3c2ac6007c66b1d` and resulting image `19f63f49969286aabfaf747d8fb4f1b6fd83a367ffb44af51141bf5f1d40af30`. Its non-root/read-only/network-disabled health assertion passed and its owned tag was cleaned. The updated [example validation](../../examples/devops/validation.json) preserves source/binary/workflow fingerprints, semantic/denial/replay assertions and all four live cases, including earlier failures. The local packaged binary SHA256 is `ae802ab15468e769297e175ce29407001b28c92055aff5431a878a7e967cdf4d`; Rust source is unchanged since db7b59b.

Live evidence remains exactly source-labeled: mini request-budget denial at 266eb44; Astra local/OCI acceptance at 29429de; independent Sol prefix at 29429de and local/OCI composite/recovery at 0eef26a; mini 01/12/19 at db7b59b and mini 20 at 9c4f6e0. All required distinct cases have passing assertions. Earlier failed full invocations remain failed. The later connection-lifecycle correction preserves provider operations and reservation semantics and has deterministic success/error/constructor closure tests; no unchanged paid case was repeated. Final known usage is 45 requests, 10,367 input+output tokens, 113 charged seconds and USD 0.044363 estimate. One incomplete Sol attempt remains charged at its full 12-request/25,536-token/240-second/USD 2 reservation. Total allowance charged is 57 requests, 35,903 tokens, 353 seconds and USD 2.044363, within the original shared allowance. The reservation is not actual usage; pricing is an estimate, not an invoice. The original ledger was never reset.

Documentation code `e2f766449d6c278247b7ab8b8fb3e92a2e5d88a3` and later evidence-only head `d4b79f833d0a9d9983a44782d86453cc2e41061e` pin exact framework `ce94443a6b0a840eb9cd0e5f5b2e128dfe8f15c8`. Local paired checks and 53 browser tests passed, with the one pre-existing duplicate mobile-search test skipped; required responsive/accessibility/search/links/anchors/Mermaid/freshness checks passed. The actual hosted ZIP (artifact 9979136405, SHA256 `117f043649be7a972d5f0a7b9e10a3e5637b81e320beb1d90a4ac398877fd0a8`) passed independent verification: 280 files including `.nojekyll`, 73 HTML pages, 64 imports and exact source hashes. Visible canonical-source boilerplate is absent from built pages; internal provenance is retained. The docs pin is intentionally unchanged by this later framework evidence-only commit.

Linked draft PRs: [framework7](https://github.com/opensourceops/agentctl/pull/7) and [docs6](https://github.com/opensourceops/opensourceops.github.io/pull/6). Remaining work is human review and separately authorized release/deployment. Deliberate product and live-integration boundaries are retained in the limitation review and final evidence JSON. No merge, release tag, package publication, public-site deployment, organization change or production mutation occurred. Future relevant code changes require affected gates on their own source; do not rerun paid cases merely to refresh evidence timestamps.

Evidence-only validation: `cargo xtask docs-verify`, `cargo xtask secret-scan` and `git diff --check` passed after the final reports and matrices were updated. No executable input differs from the tested source.
